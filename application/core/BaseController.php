...
class BaseController extends MX_Controller
{
    ...
    protected function checkIsCompleteBiodata()
    {
        $checkStatusBayar = $this->db->get_where("ch_gen_tbl_user", ['userid' =>$this->userId ])->row();

        // echo "<pre>";
        // print_r($checkStatusBayar);


        if($checkStatusBayar->is_complete_biodata > 0){
            return true;
        }
        return false;
    }

    protected function checkCompleteBiodata()
    {
        $row = $this->db->get_where("ch_gen_tbl_user", ['userid' => $this->userId])->row();

        if($row->nik == null) return false;
        if($row->fullname == null) return false;
        if($row->tempat_lahir == null) return false;
        if($row->tanggal_lahir == null) return false;
        if($row->jeniskelamin == null) return false;
        if($row->agama == null) return false;
        if($row->mobilenumber == null) return false;
    ...
    // protected public function arrayToObject($data='')
    // {
